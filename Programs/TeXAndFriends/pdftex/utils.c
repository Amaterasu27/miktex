...
#if defined(MIKTEX)
char * ptexbanner = "This is pdfTeX, Version 3.141592-1.30.6";
#endif
...
#if defined(MIKTEX)
    _TCHAR szBanner[400];
    static unsigned v[4] = { VER_FILEVERSION };
    _stprintf (szBanner,
	       _T("%s (%s %u.%u.%u)"),
	       ptexbanner,
	       _T("MiKTeX"),
	       ...
	       v[1],
	       v[2]);
    pdftexbanner = maketexstring(szBanner);
#else  /* not MiKTeX */
    s = xtalloc(SMALL_BUF_SIZE + 
                strlen(ptexbanner) + 
                strlen(versionstring) + 
...
void escapestring(poolpointer in)
{
    ...
    while (in < out) {
        ...
        if ((ch < '!') || (ch > '~')) {
            /* convert control characters into oct */
#if defined(MIKTEX)
	    sprintf ((char *)&strpool[poolptr], "\\%.3o", (unsigned int)ch);
#else
...
void escapename(poolpointer in)
{
    ...
    while (in < out) {
        ...
        if ((ch >= 1 && ch <= 32) || ch >= 127) {
            /* escape */
#if defined(MIKTEX)
	    sprintf((char *)&strpool[poolptr], "#%.2X", (unsigned int)ch);
#else
...
#if defined(MIKTEX)
        sprintf((char *)&strpool[poolptr], "%.2X",
#else
...
void getmatch(int i) {
    ...
    if (found) {
#if defined(MIKTEX)
        sprintf((char *)&strpool[poolptr], "%d", pmatch[i].rm_so);
