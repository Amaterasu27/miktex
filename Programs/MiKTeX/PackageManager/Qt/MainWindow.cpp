...
#define UNEXPECTED_CONDITION(function)				\
  FATAL_MIKTEX_ERROR (function, MIKTEXTEXT("Unexpected condition."), 0)
...
MainWindow::Unimplemented ()
{
  QMessageBox::information (
    ...
    tr("This is an unimplemented feature."));
}
...
MainWindow::EnableActions ()
{
  try
      ...
      if (SessionWrapper(true)->IsMiKTeXDirect())
	{
      ...
      for (QModelIndexList::const_iterator it = selectedRows.begin();
	it != selectedRows.end() && (enableInstall || enableUninstall);
	++ it)
      {
	PackageInfo packageInfo;
	if (! pModel->TryGetPackageInfo(pProxyModel->mapToSource(*it),
	  packageInfo))
	{
	  UNEXPECTED_CONDITION ("MainWindow::EnableActions");
	}
...
/* _________________________________________________________________________
   ...
   MainWindow::SelectInstallablePackages
   _________________________________________________________________________ */
...
MainWindow::Install ()
{
  try
  {
	...
	packageInfo))
      {
	UNEXPECTED_CONDITION ("MainWindow::Install");
      }
    ...
    int ret =
      UpdateDialog::DoModal(this, pManager.Get(), toBeInstalled, toBeRemoved);
...
MainWindow::Synchronize ()
{
  try
	...
	tr("Synchronizing the package database..."),
	tr("Cancel"),
	0,
	numSteps,
	this);
