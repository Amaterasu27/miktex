...
#define CScrollView CMyScrollView
#define CX_BORDER 1
#define CY_BORDER 1
...
#ifdef MIKTEX
namespace { BOOL _afxGotScrollLines = FALSE; }
#else
...
/* _________________________________________________________________________
   ...
   CMyScrollView::GetScrollInfo32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
BOOL
CMyScrollView::GetScrollInfo32 (int		nBar,
				LPSCROLLINFO	lpScrollInfo32,
				UINT		nMask)
{
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  ASSERT (lpScrollInfo32 != 0);
  ASSERT (lpScrollInfo32->cbSize == sizeof(*lpScrollInfo32));
  ASSERT (nMask == SIF_TRACKPOS);
  if (nMask & SIF_TRACKPOS)
    {
      SCROLLINFO scrollInfo16;
      if (! CView::GetScrollInfo(nBar, &scrollInfo16, SIF_TRACKPOS))
	{
	  return (FALSE);
	}
      lpScrollInfo32->nTrackPos =
	Round (static_cast<double>(scrollInfo16.nTrackPos)
	       * m_scrollconv[nBar]);
    }
  return (TRUE);
}
...
/* _________________________________________________________________________
   ...
   CMyScrollView::SetScrollPos32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
int
CMyScrollView::SetScrollPos32 (int	nBar,
			       int	nPos32,
			       BOOL	bRedraw)
{
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  m_scrollInfo32[nBar].nPos = nPos32;
  int nPos16;
  if (m_bBigView[nBar])
    {
      nPos16 = Round(static_cast<double>(nPos32) / m_scrollconv[nBar]);
    }
  else
    {
      nPos16 = nPos32;
    }
  ASSERT (nPos16 <= 0x7fff);
   return (CView::SetScrollPos(nBar, nPos16, bRedraw));
}
#endif

/* _________________________________________________________________________

   CMyScrollView::GetScrollPos32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
int
CMyScrollView::GetScrollPos32 (int nBar) const
{
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  return (m_scrollInfo32[nBar].nPos);
}
#endif

/* _________________________________________________________________________

   CMyScrollView::SetScrollInfo32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
BOOL
CMyScrollView::SetScrollInfo32 (int		nBar,
				LPSCROLLINFO	lpScrollInfo32,
				BOOL		bRedraw)
{
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  ASSERT (lpScrollInfo32 != 0);
  SCROLLINFO scrollInfo16 = *lpScrollInfo32;
  if (lpScrollInfo32->fMask & SIF_RANGE)
    {
      ASSERT (lpScrollInfo32->nMin == 0);
      ASSERT (lpScrollInfo32->nMax >= lpScrollInfo32->nMin);
      m_scrollInfo32[nBar].nMax = lpScrollInfo32->nMax;
      if (lpScrollInfo32->nMax > 0x7fff)
	{
	  m_bBigView[nBar] = true;
	  scrollInfo16.nMax = 0x7fff;
	  m_scrollconv[nBar] =
	    (static_cast<double>(lpScrollInfo32->nMax)
	     / static_cast<double>(0x7fff));
	}
      else
	{
	  m_bBigView[nBar] = false;
	  m_scrollconv[nBar] = 1.0;
	}
    }
  if ((lpScrollInfo32->fMask & SIF_PAGE) && m_bBigView[nBar])
    {
      scrollInfo16.nPage =
	Round(static_cast<double>(scrollInfo16.nPage) / m_scrollconv[nBar]);
    }
  return (CView::SetScrollInfo(nBar, &scrollInfo16, bRedraw));
}
#endif

/* _________________________________________________________________________

   CMyScrollView::GetScrollLimit32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
int
CMyScrollView::GetScrollLimit32 (int nBar)
{
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  return (m_scrollInfo32[nBar].nMax);
}
#endif

/* _________________________________________________________________________

   CMyScrollView::SetScrollRange32
   _________________________________________________________________________ */

#if SIXTEENBITGDI
void
CMyScrollView::SetScrollRange32 (int	nBar,
				 int	nMinPos32,
				 int	nMaxPos32,
				 BOOL	bRedraw)
{
  ...
  ASSERT (nBar == SB_HORZ || nBar == SB_VERT);
  ASSERT (nMinPos32 == 0);
  ASSERT (nMaxPos32 >= nMinPos32);
  m_scrollInfo32[nBar].nMin = 0;
  m_scrollInfo32[nBar].nMax = nMaxPos32;
  m_scrollInfo32[nBar].fMask |= SIF_RANGE;
  int nMaxPos16;
  if (nMaxPos32 > 0x7fff)
    {
      m_bBigView[nBar] = true;
      m_scrollconv[nBar] =
	(static_cast<double>(nMaxPos32) / static_cast<double>(0x7fff));
      nMaxPos16 = 0x7fff;
    }
  else
    {
  ...
  CView::SetScrollRange (nBar, 0, nMaxPos16, bRedraw);
}
#endif
#endif
