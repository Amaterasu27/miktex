/* texindex -- sort TeX index dribble output into an actual index.
   $Id: texindex.c,v 1.8 2004/10/03 20:51:20 csc Exp $
...
{
#if defined(MIKTEX)
  static char * lpszTempBase = 0;
  char szTempSuffix[10];
  if (lpszTempBase == 0)
    {
      char szTempBase[_MAX_PATH];
      char * lpsz = _tempnam(0, "txi");
      if (lpsz == 0)
	{
	  /* <todo/> */
	  xexit (1);
	}
      strcpy (szTempBase, lpsz);
      lpszTempBase = szTempBase;
    }
  sprintf (szTempSuffix, ".%d", count);
  return concat ("", lpszTempBase, szTempSuffix);
#else  /* not MiKTeX */
