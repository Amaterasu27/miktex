 ...
 ******************************************************************/
  ...
  Load_TrueType_Locations( PFace  face )
  {
    ...
    /* maxp->numGlyphs + 1 must be identical to numLocations; since */
    /* the latter is used for allocation, we change numGlyphs if    */
    /* necessary.                                                   */
    if ( face->maxProfile.numGlyphs + 1 != face->numLocations )
    {
      PTRACE2((
        "maxProfile.numGlyphs + 1 (%d) is different from numLocations (%d)!\n"
        face->maxProfile.numGlyphs + 1, face->numLocations ));
      face->numGlyphs = face->maxProfile.numGlyphs = face->numLocations - 1;
    }
