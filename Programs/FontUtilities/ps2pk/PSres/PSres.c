 ...
 *
 * Modified for ps2pk in November 1995
 * (+) added def.h (architecture dependant definitions)
 * (!) added function relative_name() for portability (otherwise
 *     MSDOS names like c:\psfonts would be considered relative)
 * (!) calls of fopen() changed into pfopen() so that an external
 *     program can install its private verbose file opener
 *
 * Modifier: Piet Tutelaers:
 */
...
#include <stdlib.h>  /* free(), malloc(), realloc(), exit(), getenv() */

/* Use pfopen() instead of fopen() so that the main program can install
 * my_fopen() when the -v (verbose) flag is set.
 */
...
#define RETURN_BUF \
	...
	if (ch == '%') {	/* Comment */
	    if (count == 0 || buf[count-1] != ESCAPECHAR) {
		buf[count] = '\0';
		do {
		    ch = getc(f);
		} while (ch != EOF && ch != '\n');
    ...
    char dontDequote;
{
	...
	if (*src == ESCAPECHAR) {
		...
		*dst = '\0';
		return 1;
	    } else if (*src == dontDequote) *dst++ = ESCAPECHAR;
	}
	*dst++ = *src++;
    }
    *dst = '\0';
    ...
    int *doubleSep;
{
    ...
    *sep = NULL;
	...
	if (*src == ESCAPECHAR) {
		...
		*dst = '\0';
		return 1;
	    } else if (*src == dontDequote && *sep != NULL) *dst++ = ESCAPECHAR;
    ...
    FILE *f;
{
	...
	if (inputline[start+len-1] != ESCAPECHAR) break;
...
#define GROW 1000
	...
	/* Next line is not UNIX specific anymore! */
        if (relative_name(sep+1) && insertPrefix && !doubleEquals)
	   addingPrefix = 1;
    ...
    char *data;
{
  ...
  HANDLE hnd;
  size_t namebufsize = strlen (dirName) + _MAX_FNAME + 2;
  char *namebuf = _alloca (namebufsize);
  int proceed;

  if (extensionLen == 0)
    extensionLen = strlen (PSRES_EXT);
  ...
  while (proceed)
	...
	{
	  namelen = strlen (find_file_data.cFileName);
	  if (namelen >= extensionLen)
	    {
	      if ((_stricmp ((find_file_data.cFileName
			      + (namelen - extensionLen)),
			     PSRES_EXT)
		   == 0)
		  && _stricmp (find_file_data.cFileName, PSRES_NAME) != 0)
		{
		  ReadAndStoreFile (dirName, find_file_data.cFileName,
				    len, readFunc, data);
		}
	    }
	}
...
#if defined(MIKTEX)
	    fncmp(pathOverride, savedPathOverride) != 0 ||
	    fncmp(defaultPath, savedDefaultPath) != 0) {
#else
...
#if defined(MIKTEX)
	    fncmp(pathOverride, savedPathOverride) != 0 ||
	    fncmp(defaultPath, savedDefaultPath) != 0) return 1;
#else
	    strcmp(pathOverride, savedPathOverride) != 0 ||
    ...
    char *data;
{
	...
	dir = path;
	do {
	    while (*pathChar == ESCAPECHAR) {
		pathChar++;
		if (*pathChar != '\0') *dir++ = *pathChar++;
            }		  

	    *dir++ = ch = *pathChar++;
	} while (ch != '\0' && ch != PATHSEP);
	if (ch == PATHSEP) *(dir-1) = '\0';

	if (*path == '\0') {
	    if (ch == PATHSEP && string != savedDefaultPath) {
...
#if defined(MIKTEX)
#define GetPath MyGetPath
#endif
static char *GetPath()
{
    static char defaultEnvironmentPath[] = PSRESOURCEPATH_DEFAULT;
...
#if defined(MIKTEX)
	addingPrefix = !ISPATH(sep+1) && insertPrefix && !doubleEquals;
#else
	/* Next line is UNIX specific! */
	addingPrefix = *(sep+1) != DIRSEP && insertPrefix && !doubleEquals;
#endif
...
/* Is <name> a relative filename? */
int relative_name(char *name) {
#if defined(MIKTEX)
   if (ISPATH(name)) return 0;
#else
   if (*name == DIRSEP) return 0;
#endif
#ifdef MSDOS
   if (*(name+1) == ':' && *(name+2) == DIRSEP && isalpha(*name)) return 0;
#endif
   return 1;
}
