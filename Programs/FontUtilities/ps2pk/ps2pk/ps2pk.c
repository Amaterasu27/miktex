...
#include <stdlib.h>
#include <string.h>
#include "PSres.h"	/* PostScript Resource lookup functions */
...
{  char *argp, c;
   ...
   while (--argc > 0 && (*++argv)[0] == '-') {
      ...
      while ((!done) && (c = *++argv[0]))  /* allow -bcK like options */
	    ...
	    add_option("-Y", argv[0]);
      	    break;
      	 case 'v':
#if ! defined(MIKTEX)		/* <fixme>sporadic access violations</fixme> */
      	    verbose = 1;
#endif
   ...
   if (argc < 1 || argc >2) {
#ifdef KPATHSEA
      msg  ("Ps2pk: version 1.6k (1992-98)\n");
#else
      msg  ("Ps2pk: version 1.6 (1992-98)\n");
#endif
   ...
   if (psfile != NULL) {
	 ...
	 if (!ps_resource(encname)) 
      ...
      if (!psfile)
         fatal("%s: PS file %s not found!\n", program_invocation_name, psname);
      if (afmname == NULL)
