/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/dvi.c,v 1.3 2005/07/03 20:02:28 csc Exp $
...
/*
 * This has been moved to here from fontmap.c
 * in favor of pdf/dvi/mpost independence.
 */
static int num_fontmaps = 0;
static int max_fontmaps = 0;
...
get_map_record (const char *tex_name, int *sfd_id)
{
  ...
  for (map_id = 0;
       map_id < num_fontmaps; map_id++) {
    if (!strcmp(tex_name, fontmaps[map_id].tex_name)) {
      *sfd_id = fontmaps[map_id].subfont_id;
      return fontmaps[map_id].record;
    }
  }

  /* Not found */
  mrec = pdf_lookup_fontmap_record2(tex_name, sfd_id);
  if (!mrec)
    return NULL;

  if (num_fontmaps >= max_fontmaps) {
    max_fontmaps += 32;
    fontmaps      = RENEW(fontmaps, max_fontmaps, struct fontmap);
  }
  fontmaps[map_id].tex_name   = NEW(strlen(tex_name)+1, char);
  strcpy(fontmaps[map_id].tex_name, tex_name);
  fontmaps[map_id].subfont_id = *sfd_id;
  fontmaps[map_id].record     = mrec;
...
close_fontmaps (void)
{
  ...
  for (i = 0; i < num_fontmaps; i++) {
    RELEASE(fontmaps[i].tex_name);
  }
  RELEASE(fontmaps);
  fontmaps     = NULL;
  num_fontmaps = max_fontmaps = 0;
}
...
dvi_locate_font (const char *tfm_name, spt_t ptsize)
{
  ...
  mrec = get_map_record(tfm_name, &subfont_id);

  /* TFM must exist here. */
  loaded_fonts[thisfont].tfm_id     = tfm_open(tfm_name, 1);
  ...
  if (!mrec) {
  ...
  } else if (subfont_id >= 0 &&
	     !mrec->enc_name) {
    font_id = vf_locate_font(mrec->font_name, ptsize);
    if (font_id >= 0) {
      loaded_fonts[thisfont].type    = VIRTUAL;
      loaded_fonts[thisfont].font_id = font_id;
      if (verbose)
	MESG("(OVF)>");
      ...
      ERROR("Could not locate a Omega Virtual Font file for '%s'.", tfm_name);
    }
  }

  /* Failed to load a virtual font so we try to load a physical font.
   * map_name is different from tfm_name in the case of subfont.
   */

  /* In PostScript and PDF, "findfont" don't care about point size,
   * however, we need this for PK font creation.
   */
  if (mrec) {
    font_id = pdf_dev_locate_font(mrec->map_name, ptsize);
  } else {
    font_id = pdf_dev_locate_font(tfm_name,       ptsize);
  }
  if (font_id < 0) {
    ...
    if (mrec && mrec->font_name) {
      ...
      WARN("Please check if kpathsea library can find this font.");
    } else {
      WARN("There are no valid font mapping entry....");
    }
...
/*
 * This should be optional pre-scanning.
 */
void
		     ...
		     double *page_width, double *page_height, char *landscape)
{
  ...
  seek_absolute(dvi_file, page_loc[page_no]);
  while ((opcode = fgetc(dvi_file)) != EOP) {
	...
	if (size < 0 ||
	    fread(buffer, sizeof(char), size, dvi_file) != size)
	  ERROR("DVI file ended prematurely");

	start = buffer; end = buffer + size;

	skip_white(&start, end);
	if (!strncmp(start, "landscape", 9)) {
	  *landscape = 1;
	} else if (!strncmp(start, "pdf:", 4)) {
	  start += 4;
	  skip_white(&start, end);
	  if (start < end && !strncmp(start, "pagesize", 8)) {
	    start += 8;
	    skip_white(&start, end);
	    while (start < end) {
	      if (start < end - 6 &&
		  !strncmp(start, "width", 5)) {
		start += 6;
		number = parse_length(&start, end);
		if (number > 0.0)
		  *page_width = number * dvi_tell_mag();
	      } else if (start < end - 7 &&
			 !strncmp(start, "height", 6)) {
		start += 7;
		number = parse_length(&start, end);
		if (number > 0.0)
		  *page_height = number * dvi_tell_mag();
	      } else {
		ERROR("Invalid key in pdf:pagesize special.");
		dump(start, end);
	      }
	      skip_white(&start, end);
	    }
	  }
	} else if (!strncmp(start, "papersize", 9)) {
	  char *key, *val;
	  ...
	  if (key && val) {
	    ...
	    number = parse_length(&val_start, val_end);
	    if (number > 0.0)
	      *page_width = number;
	    skip_white(&val_start, val_end);
    ...
    default: /* case PRE: case POST: case POST_POST: and others */
      ERROR("Unexpected opcode %d at pos=0x%x",
	    opcode, tell_position(dvi_file));
    }
  }
}
