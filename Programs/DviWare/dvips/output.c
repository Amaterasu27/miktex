...
{
#if defined(MIKTEX)
  if (systemtype && strncmp(s, "gunzip -c ", 10) == 0)
    {
      s += 10;
      systemtype = 0;
    }
  else if (systemtype && strncmp(s, "bunzip2 -c ", 11) == 0)
    {
      s += 11;
      systemtype = 0;
    }
	...
	*/
	...
	typedef int __cdecl File_Handler(FILE *, FILE *, char *name);
	typedef int __cdecl Checker_Handler(char *name);
	
	typedef struct {
	  char *suffix;
	  File_Handler *function;
	  Checker_Handler *checker;
	} Handler_Entry;
	
	static Handler_Entry handler_table[] = {
	  /* place for other handlers here */

	  /* default handler */
#ifndef Omega
#if HAVE_BMEPS_RUN
	  { "*", bmeps_run, bmeps_can_handle },
#endif
#endif
	...
	*/
	...
	static void print_filename (char * s)
	{
	  ...
	  if(!quiet) {
	    if (strlen(x) + prettycolumn > STDOUTSIZE) {
	      prettycolumn = 0 ;
	      fprintf(stderr, "\n");
	    }
	    (void)fprintf(stderr, "<%s>", x) ;
	    prettycolumn += 2 + strlen(x) ;
	  }
	}
	...
	*/
	...
	void figcopyfile (char * s, int systemtype)
	{
	  FILE *inputfile;
	  if(systemtype) {
	    /* traditional put-through for pipes */
	    print_filename(s);
	    original_figcopyfile(s, systemtype);
	  } else {
	    Handler_Entry *hefound, *herunning;
	    File_Handler  *fct;
	    char *suffix;
	    hefound = NULL;
	    suffix = get_filename_suffix(s);
	    if(suffix) {
	      int found_level;
	      herunning = handler_table; found_level = 0;
	      while((found_level < 2) && (herunning->suffix)) {
		if(strcmp(herunning->suffix, "*") == 0) {
		  if(found_level == 0) {
		    Checker_Handler *chk;
		    chk = herunning->checker;
		    if(chk) {
		      if((*chk)(s)) {
		        hefound = herunning; found_level = 1;
		      }
		    }
		  }
		} else {
	    ...
	    if(hefound) {
	      /* use the handler */
	      fct = hefound->function;
	      inputfile = search(figpath, s, READBIN) ;
	      if(inputfile == NULL) {
		inputfile = search(headerpath, s, READBIN) ;
	      }
	      if(inputfile) {
		int ok;
		print_filename(s);
		fprintf(bitfile, "\n%%%%BeginDocument: %s 1.0 bmeps\n", s);
		ok = (*fct)(bitfile,inputfile,s);
		fprintf(bitfile, "\n%%%%EndDocument\n");
		if(!ok) {
		  if(!quiet) {
		    if(prettycolumn) {
		    ...
		    fprintf(stderr, "ERROR: graphics handler failed!\n");
		    prettycolumn = 0;
		  }
		}
		fclose(inputfile);
	      ...
	      /* traditional put-through if no vaild handler found */
	      print_filename(s);
	      original_figcopyfile(s, systemtype);
	    }
	  }
...
initprinter(sectiontype *sect)
{
   ...
   if (disablecomments) {
	 ...
	 int psl;
	 bmeps_configure();
	 psl = bmeps_get_pslevel();
	 if(psl < 2) { psl = 2; }
         if (isepsf)
            (void)fprintf(bitfile, "%%!PS-Adobe-%d.0 EPSF-%d.0\n", psl, psl) ;
         else
            (void)fprintf(bitfile, "%%!PS-Adobe-%d.0\n", psl) ;
	 }
#else
         if (isepsf)
