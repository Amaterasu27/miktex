/*  $Header: /cvsroot/miktex/miktex/dvipdfm/psimage.c,v 1.9 2005/05/23 19:49:15 csc Exp $
...
{
#if defined(MIKTEX)
  char gsexe[_MAX_PATH];
  if (_strnicmp("gswin32c ", s, 9) == 0
      || _strnicmp("gswin32c.exe ", s, 13) == 0)
    {
      miktex_get_gs_exe (gsexe);
      distiller_template = NEW (strlen(s)+1 + strlen(gsexe), char);
      strcpy (distiller_template, gsexe);
      s += 8;
      while (*s != ' ')
	s++;
      strcat (distiller_template, s);
      return;
    }
#endif
...
#if defined(MIKTEX)
  miktex_create_temp_file_name (szTmp);
  tmp = szTmp;
#else
  tmp = tmpnam (NULL);
#endif
