/*  $Header: /cvsroot/miktex/miktex/dvipdfm/dvipdfm.c,v 1.11 2005/09/03 20:03:10 csc Exp $
...
static void set_default_pdf_filename(void)
{
  ...
  if (strlen(dvi_base) < 5
      || strnicmp(".dvi", dvi_base + strlen(dvi_base)-4, 4))
#else
...
static void do_args (int argc, char *argv[])
{
  ...
  if (argc > 0) {
#if defined(MIKTEX_WINDOWS)
    if (strnicmp(".dvi", argv[0] + strlen(argv[0])-4, 4)) {
#else
...
static void read_config_file (void)
{
  ...
  unsigned i;
  for (i = miktex_get_number_of_texmf_roots(); i > 0; -- i)
    {
      char szRoot[_MAX_PATH];
      char szSearchPath[_MAX_PATH + 100];
      char szConfigFile[_MAX_PATH];
      size_t n;
      miktex_get_root_directory (i - 1, szRoot);
      n = strlen(szRoot);
      if (n > 0 && szRoot[n - 1] == '/' || szRoot[n - 1] == '\\')
	{
      ...
      miktex_replace_string (szSearchPath,
			     ...
			     "%R",
			     szRoot);
      if (miktex_find_file(config_file_name, szSearchPath, szConfigFile) != 0)
	{
	  full_config_name = szConfigFile;
#else
