...
hash (const char * id)
{
  ...
  return (static_cast<SYMBOL_PTR>(h % HASH_PRIME));
}
  ...
  for (; s < NIL_SYMBOL; s = symbol_table[s].s_next)
      ...
      if (strcmp(symbol_table[s].s_repr, id) == 0) /* FIXME: ignore case */
	{
	  ...
	  if (possible_result->s_kind == C_RESERVED)
	    {
	      return (lookup(const_cast<const char *>
			     (reinterpret_cast<char *>(possible_result->s_type_ptr))));
	    }
	     ...
	     const char * mapped_name)
{
  ...
  sym->s_type_ptr
    = (const_cast<void*>
       (reinterpret_cast<const void *>(get_string(new_string(mapped_name)))));
}
