...
PkChar::gpower[33] =
{
  ...
  twopwr(28) - 1, twopwr(29) - 1, twopwr(30) - 1,
  2147483647, //twopwr(31) - 1
  -1
};
...
PkChar::PkChar (/*[in]*/ DviFont * pFont)
...
/* _________________________________________________________________________
   ...
   PkChar::GetPackedNumber
   _________________________________________________________________________ */

int
PkChar::Unpacker::GetPackedNumber ()
{
  int i = GetNybble();
  int j;
  if (i == 0)
...
unsigned long
PkChar::CountBits (/*[in]*/ const RASTERWORD *	pRasterWord,
		   /*[in]*/ int			xStart,
...
inline
unsigned long
color (/*[in]*/ unsigned long	n,
       /*[in]*/ unsigned long	bitsPerPixel,
       /*[in]*/ unsigned long	shrinkFactor)
{
  return (Round(static_cast<double>(n * (twopwr(bitsPerPixel) - 1))
		/ static_cast<double>(shrinkFactor * shrinkFactor)));
}
...
void *
PkChar::Shrink (/*[in]*/ int shrinkFactor)
{
#define BE_FAST
#ifdef BE_FAST
  if (shrinkFactor == 1)
    {
      ...
      unsigned long rasterWordsPerLine =
	(rasterWidth + bitsPerRasterWord - 1) / bitsPerRasterWord;

      unsigned char * pShrinkedRaster =
	reinterpret_cast<unsigned char*>(malloc(rasterHeight * cbLine));
  ...
  unsigned long bitsPerPixel =
    pDviFont->GetDviObject()->GetBitsPerPixel(shrinkFactor);
  ...
  unsigned char * pShrinkedRaster =
    reinterpret_cast<unsigned char*>(malloc(heightShr * lineSizeShr));
  memset (pShrinkedRaster, 0, heightShr * lineSizeShr);

  int shrinkedRasterHeight = 0;

  int sampleHeight
    = (cyOffset + 1) - ((cyOffset + 1) / shrinkFactor) * shrinkFactor;
  ...
  for (int row = 0; row < rasterHeight; shrinkedRasterHeight += 1)
    {
      ...
      int sampleWidth
	= ((cxOffset + 1)
	   - ((cxOffset + 1) / shrinkFactor) * shrinkFactor);
      if (sampleWidth <= 0)
	{
