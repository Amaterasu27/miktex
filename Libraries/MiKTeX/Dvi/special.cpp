...
/* _________________________________________________________________________
   ...
   DviImpl::PatternToShadeLevel
   _________________________________________________________________________ */

float
DviImpl::PatternToShadeLevel (/*[in]*/ const char * lpszTexture)
{
...
/* _________________________________________________________________________
   ...
   DviImpl::InterpretSpecial
   _________________________________________________________________________ */

bool
DviImpl::InterpretSpecial (/*[in]*/ DviPageImpl *	pPage,
			   /*[in]*/ int			x,
			   /*[in]*/ int			y,
			   ...
			   /*[in]*/ DviSpecial * &	pSpecial)
{
  ...
  switch (lpszSpecial[0])
    ...
    case '!':
      pSpecial =
	new DviSpecialObject<PsdefSpecialImpl> (pPage, x, y, lpszSpecial);
      ret = true;
      break;
    case '"':
      pSpecial =
	new DviSpecialObject<DvipsSpecialImpl> (pPage, x, y, lpszSpecial);
      ret = true;
      break;
    case 'a':
	...
	{
	  pSpecial =
	    new DviSpecialObject<TpicSpecialObject<TpicArcSpecialImpl> >
	    (pPage,
	     x,
	     y,
	     lpszSpecial);
	...
	{
	  pSpecial = new DviSpecialObject<GraphicsSpecialImpl> (pPage,
								x,
								y,
								lpszSpecial);
	  ret = true;
	...
	{
	  pSpecial
	    = new DviSpecialObject<TpicSpecialObject<TpicPolySpecialImpl> >
	    (pPage,
	     x,
	     y,
	     lpszSpecial);
      ...
      if (strncmp(lpszSpecial, "em:", 3) == 0)
	{
	  ...
	  if (strncmp(lpsz, "graph", 5) == 0)
	    {
	      pSpecial =
		new DviSpecialObject<GraphicsSpecialImpl> (pPage,
							   x,
							   y,
							   lpszSpecial);
	      ret = true;
	  ...
	  else if (strncmp(lpsz, "lineto", 6) == 0
		   || strncmp(lpsz, "line", 4) == 0)
	    {
	      pSpecial =
		new DviSpecialObject<SolidLineSpecialImpl> (pPage,
							     x,
							     y,
							     lpszSpecial);
	      ret = true;
	...
	{
	  pSpecial =
	    new DviSpecialObject<PsdefSpecialImpl> (pPage,
						    x,
						    y,
						    lpszSpecial);
	  ret = true;
	...
	{
	  pSpecial =
	    new DviSpecialObject<LandscapeSpecialImpl> (pPage,
							x,
							y,
							lpszSpecial);
	  landscape = true;
	...
	{
	  if (sscanf_s(lpszSpecial, "pn %d",
		       &TpicSpecialRoot::tpicContext.penSize)
	      != 1)
	    {
	      trace_error->WriteFormattedLine
		("libdvi",
      ...
      else if (strncmp(lpszSpecial, "papersize=", 10) == 0
	       || strncmp(lpszSpecial, "papersize ", 10) == 0)
	{
	  pSpecial =
	    new DviSpecialObject<PaperSizeSpecialImpl> (pPage,
							x,
							y,
							lpszSpecial);
	  havePaperSizeSpecial = true;
	  paperSizeInfo =
	    dynamic_cast<PaperSizeSpecial*>(pSpecial)->GetPaperSizeInfo();
	  ret = true;
	}
      else if (strncmp(lpszSpecial, "pa", 2) == 0)
	{
      ...
      else if (strncmp(lpszSpecial, "PSfile=", 7) == 0
	       || strncmp (lpszSpecial, "psfile=", 7) == 0)
	{
	  pSpecial =
	    new DviSpecialObject<PsfileSpecialImpl> (pPage,
						     x,
						     y,
						     lpszSpecial);
	  ret = true;
	...
	{
	  pSpecial =
	    new DviSpecialObject<DvipsSpecialImpl> (pPage,
						    x,
						    y,
						    lpszSpecial);
	  ret = true;
	...
	{
	  pSpecial =
	    new DviSpecialObject<SourceSpecialImpl> (pPage,
						     x,
						     y,
						     lpszSpecial);
	  ret = true;
	...
	{
	  if (sscanf_s(lpszSpecial, "sh %f",
		       &TpicSpecialRoot::tpicContext.shade)
	      != 2)
	    {
	      trace_error->WriteFormattedLine
		("libdvi",
	...
	{
	  TpicSpecialRoot::tpicContext.shade =
	    PatternToShadeLevel(lpszSpecial + 2);
	  ret = true;
	}
      break;
    case 'w':
  ...
  if (! ret)
    {
      trace_error->WriteFormattedLine ("libdvi",
...
SolidLineSpecialImpl::Parse ()
{
  ...
  if (strncmp(lpsz, "lineto", 6) == 0)
      ...
      if (isdigit(*lpsz))
	{
	  float texWidth;
	  char unit[3];
	  sscanf_s (lpsz, "%f%2s", &texWidth, unit);
