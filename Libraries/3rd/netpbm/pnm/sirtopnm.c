...
char* argv[];
{
    ...
    if ( info != 0x3a4f)
	pm_error( "Input file is not a Solitaire file", 0,0,0,0,0 );
    pm_readlittleshort( ifp, &info );
    pm_readlittleshort( ifp, &info );
    if ( info == 17 )
    {
	...
	pm_error( "Input is not MGI TYPE 11 or MGI TYPE 17", 0,0,0,0,0 );
    pm_readlittleshort( ifp, &info );
    cols = (int) ( info );
    pm_readlittleshort( ifp, &info );
    rows = (int) ( info );
    ...
    {
	case PGM_TYPE:
            pm_message( "Writing a PGM file", 0,0,0,0,0 );
	...
	case PPM_TYPE:
	    ...
	    planesize = cols * rows;
	        ...
	        pm_error( "Not enough memory to load SIR file", 0,0,0,0,0 );
	    if ( fread( sirarray, 1, picsize, ifp ) != picsize )
	        pm_error( "Error reading SIR file", 0,0,0,0,0 );
            pm_message( "Writing a PPM file", 0,0,0,0,0 );
            for ( row = 0; row < rows; row++ )
	    {
	        for ( col = 0, xP = xelrow; col < cols; col++, xP++ )
