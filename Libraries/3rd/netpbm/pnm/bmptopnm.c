/*\
 * $Id: bmptopnm.c,v 1.1.1.1 2000/09/07 13:01:23 csc Exp $
 * 
 * bmptopnm.c - Converts from a Microsoft Windows or OS/2 .BMP file to a
 * PNM file.
 * 
 ...
 *
 * Modified by Fujita, Isao (fujita@itolab.ei.tuat.ac.jp), Oct 27, 1996.
 * 
 * Added support for 24-bits BMP files as input and for portable anymap
 * as output, the program is renamed bmptopnm.c from bmptoppm.c.
 * 
 * 
 * $Log: bmptopnm.c,v $
 * Revision 1.1.1.1  2000/09/07 13:01:23  csc
 * Import of MiKTeX 2.0 beta
 *
 * Revision 1.1.1.1  1998/08/13 08:40:08  mik
 * netpbm import
 *
...
static int BMPreadrgbtable ARGS((FILE *fp, unsigned long *ppos,
    unsigned short cBitCount, int class, xelval *R, xelval *G, xelval *B,
    int *pnm_format));
static int BMPreadrow ARGS((FILE *fp, unsigned long *ppos, xel *row,
    unsigned long cx, unsigned short cBitCount, xelval *R, xelval *G, xelval *B));
static xel ** BMPreadbits ARGS((FILE *fp, unsigned long *ppos,
 ...
 */
static int
BMPreadrgbtable(fp, ppos, cBitCount, class, R, G, B, pnm_format)
	FILE           *fp;
	unsigned long  *ppos;	/* number of bytes read from fp */
	unsigned short  cBitCount;
	int             class;
	...
	int            *pnm_format;
{
	...
	for (i = 0; i < ncolors; i++)
	{
		B[i] = (xelval) GetByte(fp);
		G[i] = (xelval) GetByte(fp);
		R[i] = (xelval) GetByte(fp);
		nbyte += 3;

		grayflag &= (B[i] == G[i] && G[i] == R[i]);
		monoflag &= (G[i] == 0 || G[i] == MAXCOLORS-1);
	...
	*pnm_format = (monoflag) ? PBM_TYPE :
		      (grayflag) ? PGM_TYPE : PPM_TYPE;
	...
	xelval         *B;
{
	...
	if ((b = pm_bitinit(fp, RB)) == (BITSTREAM) 0)
	{
		return -1;
	}
	...
	else
	{
		for(x = 0; x < cx; x++, row++)
		{
			...
			blu = (xelval)((v & 0x00ff0000) >> 16);
			grn = (xelval)((v & 0x0000ff00) >> 8);
			red = (xelval)((v & 0x000000ff));

			PPM_ASSIGN(*row, red, grn, blu);
		}
	}
	...
	xelval         *B;
{
	...
	{
		int rc;
		rc = BMPreadrow(fp, ppos, xels[y], cx, cBitCount, R, G, B);
	...
	char          **argv;
{
	...
	xelval          R[MAXCOLORS];	/* reds */
	xelval          G[MAXCOLORS];	/* greens */
	xelval          B[MAXCOLORS];	/* blues */
	...
	rc = BMPreadrgbtable(ifp, &pos, cBitCount, class, R, G, B, &pnm_format);
	...
	xels = BMPreadbits(ifp, &pos, offBits, cx, cy
	...
	pm_close(ifp);
	pnm_writepnm(stdout, xels, cx, cy, (xelval) (MAXCOLORS-1), pnm_format, 0);
	pm_close(stdout);
