#ifndef lint
static char *RCSid = "$Id: pnmtoddif.c,v 1.2 2004/09/30 19:38:13 csc Exp $";
#endif

/*
 * $Log: pnmtoddif.c,v $
 * Revision 1.2  2004/09/30 19:38:13  csc
 * *** empty log message ***
 *
...
static void tag ARGS((unsigned char **, int, int, unsigned int));
static void ind ARGS((unsigned char **));
static void wr_null ARGS((unsigned char **));
static void wr_length ARGS((unsigned char **, int));
static void wr_int ARGS((unsigned char **, int));
static void eoc ARGS((unsigned char **));
static void wr_string ARGS((unsigned char **, char *));
static void emit_isolatin1 ARGS((unsigned char **, char *));
int write_header ARGS((FILE *, imageparams *));
int write_trailer ARGS((FILE *));
...
static void tag(buffer,cl,constructed,t)
...
static void wr_length(buffer,amount)
unsigned char **buffer;
int amount;
{
  int length;
...
static void wr_string(buffer,val)
unsigned char **buffer;
char* val;
{
  int length;
...
static void emit_isolatin1(buffer,val)
unsigned char **buffer;
char* val;
{
  int length;
