...
char *argv[];
    ...
    int warned = 0;
    int *obuf;
    char *usage = "[-d <val>] height width [asciifile]";
     ...
     */
    if ((obuf = (int *) calloc (cols, sizeof (int))) == NULL)
	pm_error( "malloc error", 0,0,0,0,0 );		/* wimpy message! */
    ...
    while ( row < rows )
	{
		...
		if (c > 0x7f)		/* !isascii(c) */
		    ...
		    if (!warned)
			{
			fprintf (stderr, "(Warning: non-ASCII char(s) in input)\n");
			warned = 1;
			}
		    c &= 0x7f;		/* toascii(c) */
		    }
