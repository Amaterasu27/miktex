...
static void slider ARGS((void (*slvec) ARGS((struct svector *vec, int colour)),
                    void (*slflood) ARGS((struct spolygon *poly, int colour)) ));
static void slidefind ARGS((char *sname, int dironly, int ucasen));
static void draw ARGS((struct svector *vec, int colour));
static void flood ARGS((struct spolygon *poly, int colour));
...
static void slider(slvec, slflood)
  void (*slvec) ARGS((struct svector *vec, int colour));
  void (*slflood) ARGS((struct spolygon *poly, int colour));
{
    int i, rescale;
  ...
  int dironly, ucasen;
{
    ...
    if ((fread(libent, 32, 1, slfile) != 1) ||
        (strcmp((char *)libent, "AutoCAD Slide Library 1.0\015\012\32") != 0)) {
	...
	if (dironly) {
            pm_message("  %s", libent);
	} else if (strcmp((char *)libent, uname) == 0) {
