/*\
 * $Id: ppmtobmp.c,v 1.2 2004/09/30 19:38:14 csc Exp $
 *
 ...
 *
 * Added support for 24-bits BMP files as output.
 *
 *
 * $Log: ppmtobmp.c,v $
 * Revision 1.2  2004/09/30 19:38:14  csc
 * *** empty log message ***
 *
        ...
        colorhash_table cht;
{
        ...
        if ((b = pm_bitinit(fp, WB)) == (BITSTREAM) 0)
#else
        ...
        for (x = 0; x < cx; x++, row++)
        {
                if (bpp != 24)
                {
                        rc = pm_bitwrite(b, bpp, ppm_lookupcolor(cht, row));
                }
                else
                {
                        long    v = (PPM_GETB(*row) << 16)
                                  + (PPM_GETG(*row) << 8) + PPM_GETR(*row);
                        rc = pm_bitwrite(b, bpp, v);
                }
        ...
        int             colors;
{
        ...
        if      (colors <= 2)
                ...
                bpp = 8;
        else if (colors <= 16777216L)
                bpp = 24;
        else
                pm_error("colors of %d is too large!", colors);
