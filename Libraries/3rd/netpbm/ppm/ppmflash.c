...
char *argv[];
{
	...
	for (i = 0; i < rows; i++)
	{
		...
		for (j = 0; j < cols; j++)
		{
			PPM_ASSIGN(*pP2, PPM_GETR(*pP) + (((maxval - PPM_GETR(*pP)) * longfactor) >> 16),
							 PPM_GETG(*pP) + (((maxval - PPM_GETG(*pP)) * longfactor) >> 16),
							 PPM_GETB(*pP) + (((maxval - PPM_GETB(*pP)) * longfactor) >> 16));
